...
| Interface, battle and reward SFX — 64 cues | Synthesised in the browser from `soundCue` content by the game's own engine (`packages/shared/src/audio`); no files, no third party | n/a — nothing to licence | In use (P10c; rebuilt C51) |
| Music (2 tracks) and the Wardenmaster's narration (12 lines), plus `wardenmaster_avatar.jpg` | The owner's own files — `assets/music_and_sounds/`, `assets/ui/misc_avatars/` | Owner's own work (all rights held) | In use (2026-08-21) |
