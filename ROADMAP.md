...
### P2 — Battle engine ◐ **engine + balance-sim complete; battle API next**
...
**Still owed:** the battle API (start/action/auto/retreat/active/log with session snapshots and idempotency) and the Admin battle inspector — both need the player roster that arrives with P4, so the shape of a "team" is settled first.
