...
describe('speedsFor', () => {
  ...
  it('opens ×4 on a finished Normal campaign and nothing more', () => {
    expect(speedsFor({ normal: true })).toEqual([1, 2, 4]);
  });

  it('opens ×6 on Brutal, which is the top of the ladder', () => {
    expect(speedsFor({ normal: true, brutal: true })).toEqual([1, 2, 4, 6]);
  ...
  it('does not hand out ×4 for finishing Brutal alone', () => {
    ...
    expect(speedsFor({ brutal: true })).toEqual([1, 2, 6]);
  });
  ...
  it('takes its pairing from config rather than from this file', () => {
    ...
    expect(speedsFor({ normal: true }, { '2': 'normal', '4': 'brutal' })).toEqual([1, 2, 6]);
    expect(speedsFor({ hard: true }, { '4': 'hard' })).toEqual([1, 2, 4, 6]);
  });
  ...
  it('never leaves an account with no speed at all', () => {
    ...
    expect(speedsFor({}, { '1': 'brutal', '2': 'brutal', '4': 'brutal', '6': 'brutal' })).toEqual([
      1,
    ]);
  });

  it('ships the pairing the owner asked for as the default', () => {
    expect(DEFAULT_SPEED_UNLOCKS).toEqual({ '4': 'normal', '6': 'brutal' });
  });
});
...
describe('clampSpeed', () => {
  ...
  it('falls back to the fastest open rung rather than to ×1', () => {
    ...
    expect(clampSpeed(6, [1, 2, 4])).toBe(4);
  });
