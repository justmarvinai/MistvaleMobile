...
- **The champion benchmark (C29):** every champion is fought in one **fixed** team on one **fixed** stage at one **fixed** investment — slot 0 under test, the other three the same trio every time — and scored on the **turns the team took to clear**. Turns rather than any per-champion figure, and that was settled by measuring: scoring the fighting roles on damage and `support` on healing plus shielding reads as noise, because **six of Mistvale's ten supports neither heal nor shield** — they buff, debuff and move the turn meter, and six of ten came out at zero. Turns is defined for every kit, is the currency the game already records `bestTurns` in, and credits each role in its own coin: an attacker shortens the fight by killing, a tank by keeping the party alive to keep killing, a support by making the other three better at it. The bench is the last Brutal campaign stage, chosen for having **no boss mechanic** — the Titan and the world boss are the obvious dummies and both carry a hit-counter shield, which would turn the table into "is this champion a multi-hitter".
  - **What is enforced** is three statements that hold whatever the tuning turns out to be: the bench fight is still winnable (or every turn count below it is meaningless), **every champion does something** — damage, healing or shielding, and survives some of the time — and **no champion exceeds 200% of its role's median**. All three are mutation-checked and each fires on a different mutation.
  - **The 85–115% band is not enforced, and that is now a decision rather than a gap** (Q9, answered 2026-08-30: the roster is left alone and the band stays unenforced). The roster spans **75–151%** as shipped and ten of the thirty-seven sit outside the documented band — but a spread like that within a role is not obviously wrong for a collection game, and the ten are champions doing what they were authored to do. The report is printed on every run, so a champion pass has its input the day anybody wants one. Two observations from it are worth carrying: **Ugrim Pyrechant is authored `support` and out-damages half the `attack` roster**, and **six of the ten supports neither heal nor shield**.
  - **There is no lower bound**, deliberately. The score is what the *team* did and the other three carry the fight, so the measure saturates from below: a champion stripped to one point of attack, health and defence — which dies on the first wave and does nothing — still scores **66%**, against a roster minimum of 73%. Any floor far enough from 73% to clear the sampling noise would sit under 66% and could never fire, and a guard that cannot be made to fail has not been checked. "Does something" covers that end instead.
- **Arena diversity (C29b):** random four-champion comps at **identical** level, rank, ascension and relics, fought in arena mode — and every pairing fought **twice with the sides swapped**, because the attacker moves first and that is worth real win rate. At the default that is six thousand battles. What is reported is each champion's own win rate; what is **gated** is each *role's*, pooled.
  - **The gate is not the >40%-of-winning-comps figure this doc has named since P2**, and that is arithmetic rather than a shortcut. A champion fills 4 of 37 comp slots, so it is in **10.8%** of random comps; every battle has exactly one winner, so half of all comps win. A champion that won *every* fight it ever appeared in would still be in only 0.108 / 0.5 = **21.6%** of winning comps — the 40% line could never be crossed. That figure means what it means in a metagame where players *choose* their comps; here they are drawn, so the same question is asked in the form drawn comps can answer.
  - **Nor is it per-champion**, for the reason C29's benchmark has no lower bound: one champion in four cannot escape the noise of its three random partners. A champion given **a hundred times** its authored attack tops the table at 76.5%, against an authored best of 76.3% — indistinguishable — and stripped to 1/1/1 it falls only to 20.6% against an authored worst of 24.9%. Nothing could cross a champion-level bound.
  - **A role can.** Pooling six to fourteen champions over thousands of battles takes the noise with it: a role's figure moves **0.2–1.3 points** across disjoint seed blocks. Shipped, the roles run **attack 42.5% · defense 51.4% · hp 54.0% · support 56.9%**, and the gate is a **ceiling of 62%** — five points clear of the maximum, five times the drift, and demonstrably crossable (every support at five times its attack reaches 65.9%). There is **no floor**: a role can be pushed up but not down, because a comp holding a crippled attacker still holds three others who win it — every attack champion cut to a *twentieth* of its attack moves the role only from 42.5% to 38.5% and stops. A second gate checks the *draw* rather than the content: every champion must actually be fielded, which is the one thing about this harness that could silently go wrong and still print a plausible table.
  - The finding worth carrying is that **the Arena inverts the stage benchmark**: supports are the strongest role in PvP and the slowest at clearing PvE, and attackers are the reverse. That is two modes rewarding different champions, which is what a collection game wants.
