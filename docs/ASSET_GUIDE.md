...
| Audio (SFX) | **Synthesised, no files** — each cue is a `soundCue` content entry naming a bus and a `patch`: two to six **layers** (source, pitch sweep, AHDSR, resonant filter with its own sweep, unison, harmonics, saturation, pan) plus a stereo room, an echo and a per-play variation, rendered in the browser by `packages/shared/src/audio/synth.ts` (P10c, rebuilt in C51). Nothing to licence, nothing to credit, nothing to download, and every number retunable in Admin. Sound is the one content nobody can review in a diff, so the seed's test renders every cue and pins each family's character, and `pnpm --filter @mistvale/server sounds:render` writes the catalogue to WAV for listening. A `sample` field on the same entry takes a recording instead: publish a file under `apps/client/public/`, name its path in the cue, and the synth stops being used for that cue alone — the mixer decodes and plays it on the same bus. **Confirmed by the owner on 2026-08-18** (USER_QUESTIONS Q4) and reaffirmed by the C51 rework: synthesised is the voice of the game, and every recorded source is in any case unreachable from the build environment. |
...
## 4. Pipeline recap
Build: `tools/asset-sync` (`pnpm assets`) copies `assets/champions|enemies/<unit>/` into `apps/client/public/sprites/<kind>/<unit>/` with names the client can build a URL from (`idle/frame_000.png…`, `still.png`, `avatar.png`) — **resizing the avatars on the way** and nothing else. They are delivered at 1254×1254 and drawn at 150px on a champion card and 44px on an arena portrait, so the eight of them were 14 MB of a 15 MB tree: the largest single thing a player downloads on a 1-core box, by an order of magnitude, growing with every champion that gets a face (Q6). They go out at **320px**, twice the largest place any of them is drawn, and the tree is 2.0 MB. `assets/` keeps the masters byte-for-byte. The resizer is `tools/asset-sync/src/png.ts` and has **no dependencies** — Q6 assumed this needed `sharp` and a native module on the VPS, and it does not: a PNG is a zlib stream with five per-row filters in front of it, and Node ships zlib. **JPEGs are resized too** (C18), by `tools/asset-sync/src/jpeg.ts` — a baseline decoder and encoder, also with no dependencies. It is a separate file from `png.ts` because nothing about the two formats is shared below the pixel buffer: a JPEG is a quantised DCT bitstream rather than a filtered zlib one. Re-encoding a painted JPEG *as* PNG is not the way out and it is worth knowing why — thousands of distinct colours with no flat regions is the case PNG is worst at, and the same picture costs about three times as much as a PNG at half the resolution. Progressive, arithmetic-coded, 12-bit and CMYK files are refused by name rather than mis-decoded. The two media sets that carry one are the **scenery** (`assets/ui/backgrounds/haven_bgs` → `/scenery`, published at 1600px: 8.4 MB of masters becomes 764 KB, and the title screen draws one of them) and the **portraits** (`assets/ui/misc_avatars` → `/portraits`, 640px; the Wardenmaster is delivered at 2048² and drawn at 260). A set may also state **`format: 'jpeg'`** and be re-encoded whatever its masters are, which the **wallpapers** do (`assets/wallpapers` → `/wallpapers`, 1600px): the owner's six tab paintings arrive as PNG and come to 12.9 MB published, against **1.45 MB** as JPEG — the same argument as the scenery, met from the other side, and the extension is rewritten with the format because a `.png` that is really a JPEG is a name somebody debugs twice. A source with **real transparency is refused** rather than flattened onto black, since JPEG has no alpha and a wrong picture published quietly is exactly what these codecs exist not to do; the check is `png.ts`'s own `isOpaque`, which the encoder has used since C16 to decide whether the alpha channel earns its byte. Sprite frames are still not resized: an idle frame is 4 KB and drawn at its own size, so shrinking one would change how the game looks rather than what it weighs. It also writes a `manifest.json` recording each unit's real frame count, so the client reads how many frames a unit *has* rather than trusting a content field that could drift. It runs from the client's own `dev` and `build`, and the published tree is gitignored: it is generated output, not a second copy of the art. `--check` (`pnpm assets:check`) answers "is my published tree current?" without rewriting it.

A set may also state **`trim: true`**, which crops the transparent margin off before the ceiling is applied, and the **brand** set is why it exists (`assets/ui/brand` → `/brand`, 840px): a logo is a *shape* rather than a scene, so it is exported centred on whatever canvas the artist was working in — the Mistvale wordmark arrives 1935×812 with its ink inside 1572×433 — and that padding is not neutral once CSS lays the box out, because the box is what is sized and spaced. Published whole, a fifth of the title screen's brand block would be nothing that can be looked at or spaced against. The threshold is the whole design of it: the honest definition of an empty edge is `alpha === 0`, and on this file it crops **nothing**, because the export carries a whisper of alpha 1 across almost the entire canvas. A pixel at alpha 1 moves a channel by less than half of one 8-bit step — invisible under any ground on any display — so the bar is the smallest one that is above nothing at all, and anything an eye could resolve is kept. A picture that is transparent to the last pixel is refused rather than published as something empty. The ceiling is the scenery's argument rather than the avatars': a soft glowing wordmark drawn at 520px goes out at **840** (183 KB) rather than a strict 2× (1120px, 292 KB), because the last half-multiple is invisible on soft art and the title screen's whole art budget is 500 KB.

The same tool publishes four folders of **finished media**, and does the opposite of what it does to sprites: it copies them under their own filenames, because content points at them *by name*. `assets/music_and_sounds/background_music/` → `public/audio/music/`, `assets/music_and_sounds/tutorial_sounds/` → `public/audio/tutorial/`, `assets/ui/misc_avatars/` → `public/portraits/`, `assets/ui/brand/` → `public/brand/`, `assets/wallpapers/` → `public/wallpapers/` (as JPEG). Every target is gitignored for the reason the sprites are, only more so: the two music tracks alone are 16 MB, and a second copy of them in git would be 16 MB nobody can edit. A source folder that is not there publishes nothing and says so — silence is a supported state, not an error path, because three tutorial steps have no recording on purpose. Later, `tools/atlas-pack` packs frames → hashed atlases in `apps/client/public/atlases/` + manifest. Runtime additions: Admin upload → server-side pack → `/uploads` → `asset_defs` row → content publish → clients pick it up on next bundle fetch. GIFs in the repo are previews only — the game always renders from frame PNGs/atlases, never GIFs.
