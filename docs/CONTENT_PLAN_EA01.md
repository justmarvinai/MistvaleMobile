...
- **Tutorial (scripted, `tutorialStep` content — shipped P9):** the Wardenmaster, a Hollowborn lantern-keeper, walks a new warden from the first mist to the point the Valewarden's Path takes over. **Fifteen steps** as seeded: 1 the cold open (three borrowed starters against a Sskarn ambush) → 2 the Haven → 3 starter choice (permanent) → 4 clear 1-1 → 5 first summon (granted Faded Sigils ×2) → 6 equip a relic (granted, not dropped) → 7 upgrade one to +1 → 8 clear 1-2 → 9 quests intro (claim one) → 10 clear 1-3 → 11 level a champion three times + rank-up explainer → 12 clear 1-4 → 13 the Bazaar (buy one) → 14 clear through to Vrash the Fenblade at 1-7 → 15 the Depths tease and the Path. The **cold open** is a `tutorial`-mode stage carrying its own team (`presetTeam`), so the borrowed starters are never minted into a roster that is about to keep one of them.
  ...
  - The XP paid along the way is sized against the account curve to clear each feature gate **before** the step that needs it (level 2 by step 3 for the calendar, 3 by step 5 for the forge, 4 by step 7 for quests, 5 by step 9 for the Bazaar) — a step pointing at a locked screen is a step nobody can finish. The cold open pays nothing; it is borrowed, and so is everything in it.
