...
- **A control that is not earned yet is not offered.** Skip appears on the battle screen only for a stage this account has already beaten, and the speed button cycles only the rungs it has earned — both from the server's answer, never from a map the client happens to be holding. Neither is refused at the mutation, because neither changes an outcome: a skipped fight resolves identically and a multiplier only divides a delay. The speed control takes the *whole* ladder with the unearned rungs passed as `lockedSpeeds`, which is the library's own vocabulary for a progression gate.
