...
| H3 | **Single-panel screens** — Sunken Stair, Wurm Wakes, Valewurm, Expeditions, Bazaar, Trials | One panel in the middle of the frame and nothing else: a 1,300px-wide GO DOWN button under a two-line lore paragraph; a Valewurm panel holding one sentence; three expedition cards over 600px of nothing; four Bazaar stalls with 90px art; a fourth trial orphaned on its own row. | ✔ Expeditions: a painting on each card, cards filling their row, the button at the foot. Trials: four across, the lent team one to a row with whole names beside 56px faces. Bazaar: 150px art, a price plate, four to a row. The Valewurm, the Wurm Wakes and the Stair: `ui/Hero` — the mode’s painting as a tall block with the way in at its foot, the ladder as `ui/Ladder` tiles beside it, the Wurm’s bar at 36px across the creature. |
...
| H7 | **Hub with one row** (Champions) | Four cards centred in the frame with 300px empty above and below. | ✔ Cards carry a live line off `readiness` (roster count, vault fill, Chronicle met, wardens kept, tokens, keys, springs), the rows share the frame and the painting takes the spare height, capped at 30rem per card. Found on the way: the Wardens card named a painting the library does not have and had drawn nothing since C12; a test refuses that now. |
| H8 | **Text volume** | Every screen has a tagline *and* section sentences: the Depths says three sentences before its first button, the Stair opens with a lore paragraph, the Pass and both calendars explain themselves in prose above the thing they are. | ✔ The Depths’ section sentences are gone; the Pass’s, both calendar tracks’ and the Stair’s own words are behind the **i**. |
