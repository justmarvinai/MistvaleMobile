...
### `players` (1:1 account)
`id, account_id unique, profile_name citext unique, level, xp, energy int, energy_updated_at, xp_boost_until timestamptz null, silver bigint, crystals, valor_medals, roster_capacity, vault_slots, tutorial_step, tutorial_progress, tutorial_action_id, tutorial_skipped, settings jsonb, summon_pity jsonb, last_summon_action_id, last_vault_action_id, daily_counters jsonb, daily_counters_day, last_multi_battle jsonb, last_daily_reset_at, is_bot bool default false` — bots are players; every system (arena, leaderboards) works on them uniformly. Currencies as columns (hot, small); stackable items normalized below. `vault_slots` is what the player has *bought*, not the capacity — the base and the ceiling are `game_config`, so an operator raising the base raises everybody's vault without touching a single player row (Q5).

`energy` is **`integer` and may sit far above its cap** (C24). The cap governs regeneration and nothing else — the clock stops filling at it and a reward does not — so `energy = 3400` against a cap of 20 is an ordinary state rather than corruption, and nothing above the cap drains faster. It was `smallint` until energy became a payable reward; the first week alone hands a newcomer a few thousand and a single operator mail can hand out more than 32,767. Anything that *writes* the column has to stamp `energy_updated_at` with the instant the value was reached (`computeEnergy().settledAt`), which carries the unfinished part of the current tick rather than rounding it away.

`xp_boost_until` is the whole of the champion-XP boost: one timestamp, null for an account that has never had one and a past value for one that has run out. There is no row per boost and nothing sweeps expired ones, because a boost is a *duration* — granting more moves the timestamp forward (`extendXpBoost`), and whether it is running is a question for the clock.
