...
**A release that changes a *nested* field of already-published content needs `--replace`.** The plain seed adds missing entities and backfills missing **top-level** fields; it never reaches inside one that is already there. So a release that adds a drop to `stage.rewards.drops.items`, an offer to `shop.offers`, or a floor's loot to a `dungeon` ships the code for it and leaves the live rows exactly as they were — no error, no warning, and a faucet that simply never runs. There are two worked examples. C6: the Mistbrew and the Waking Shard published fine as new `item` entities, and neither dropped anywhere until `SEED.sh --replace stage,dungeon,shop` ran. C13 is the sharper one, because nothing about it looks like content at all — it honours `stage.waves[].stars` in the engine, so the *code* is the release and the difficulty of 252 live campaign stages is what actually moves, and only `SEED.sh --replace stage` moves it. A release whose headline is an engine change can still be a content release. Name the affected families in the release's CHANGELOG entry and run the replace as step 2b, before the smoke test. **C51 is another top-level one**: every `soundCue` gained a `patch` and lost its `voice`, and a row published under the old shape is *lifted* as it is read — so a release that skips `SEED.sh --replace soundCue` does not go silent, it simply keeps sounding like the prototype it replaced, which is the quietest possible way for a release to have not happened. **C50 is the plainest of them**: it rewrites all 28 `item` descriptions, which is a *top-level* field on entities that already exist — so the plain seed leaves every one of them as it was, the new tooltips are drawn perfectly and say the old flavour, and nothing anywhere reports a problem. `SEED.sh --replace item`. **C24 is a third example and the widest so far**: energy and hours of XP boost became reward scalars, and the rewrite touched `loginTrack`, `mission`, `quest`, `event` and `shop` — every one of those is a *nested* `rewards` map on an entity that already exists, so the release needs `SEED.sh --replace loginTrack,mission,quest,event,shop` or a new account's first week pays exactly what it paid before and the whole feature is invisible.
