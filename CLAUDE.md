...
## Project state
**Phases P0–P3 complete → next is P4 (champions & relics)** — see `ROADMAP.md`. The whole core loop runs: a fresh account registers, picks a starter, walks the campaign map, fights a stage manually or on auto watching it play out, and is paid for it. The monorepo, CI, database, auth, deploy scripts, content pipeline, the full 37-champion + 6-food seed, chapters 1–3, the Admin Suite's core editors, the battle engine (`packages/engine`, with golden replays and the `pnpm sim` balance gates), the battle API and the game screens all exist and are tested. Roster/champion-detail screens, levelling, rank-up, ascension, gear and the Bazaar do not yet. When the docs and reality diverge, fix one or the other in the same PR — stale docs are a review-blocker.
