...
## Project state
**Phases P0–P5 complete; P6 in progress (the Depths are open)** — see `ROADMAP.md`. Both loops run: a fresh account registers, picks a starter, walks the campaign map, fights a stage manually or on auto, is paid for it — and then equips what dropped, upgrades it, levels and ranks and ascends champions, and shops. Farm → equip → upgrade → stronger → farther is closed, and the Mistgate is open. The monorepo, CI, database, auth, deploy scripts, content pipeline, the full 37-champion + 6-food seed, the whole campaign (12 chapters × 7 stages × 3 difficulties, twelve warlords), the Admin Suite's core editors, the battle engine (`packages/engine`, with golden replays and the `pnpm sim` balance gates), the battle API, the game screens, the relic economy, champion progression, the Bazaar, the Mistgate, the Chronicle, the Depths — four relic keeps, the Proving Grounds and five rotating Essence Springs, with the boss mechanics that make them puzzles — masteries (48 nodes, three trees, engine-backed), and multi-battle with the practice sandbox all exist and are tested. The Arena, quests and the tutorial do not yet. When the docs and reality diverge, fix one or the other in the same PR — stale docs are a review-blocker.
