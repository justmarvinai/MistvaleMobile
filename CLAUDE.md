...
## Project state
**Phases P0–P4 complete → next is P5 (summoning)** — see `ROADMAP.md`. Both loops run: a fresh account registers, picks a starter, walks the campaign map, fights a stage manually or on auto, is paid for it — and then equips what dropped, upgrades it, levels and ranks and ascends champions, and shops. Farm → equip → upgrade → stronger → farther is closed. The monorepo, CI, database, auth, deploy scripts, content pipeline, the full 37-champion + 6-food seed, chapters 1–3, the Admin Suite's core editors, the battle engine (`packages/engine`, with golden replays and the `pnpm sim` balance gates), the battle API, the game screens, the relic economy, champion progression and the Bazaar all exist and are tested. Summoning, the Depths, Arena, quests and the tutorial do not yet. When the docs and reality diverge, fix one or the other in the same PR — stale docs are a review-blocker.
